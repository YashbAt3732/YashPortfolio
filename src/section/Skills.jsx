...
import { FaReact, FaNode, FaGithub, FaPython } from "react-icons/fa";
import { SiTailwindcss, SiMongodb, SiExpress, SiFirebase, SiFlutter } from "react-icons/si";
...
const Skills = () => {
  const skills = [
    ...
    { icon: <FaPython />, name: "Python" }, // fixed
    { icon: <FaNode />, name: "Node.js" },
    { icon: <SiTailwindcss />, name: "Tailwind CSS" },
    { icon: <SiFlutter />, name: "Flutter" }, // fixed
    { icon: <SiMongodb />, name: "MongoDB" },
    { icon: <SiExpress />, name: "Express" },
    { icon: <FaGithub />, name: "GitHub" },
    { icon: <SiFirebase />, name: "Firebase" }, // fixed
    { icon: <DiMysql />, name: "MySQL" },
    { icon: <IoLogoJavascript />, name: "JavaScript" }, // fixed
  ];
  ...
  useEffect(()=>{
    ...
    const tick=(now)=>{
      const difft=(now-last)/1000;
      last=now;

      let next=x.get()+SPEED*dir*difft;
        ...
        >
          {repeated.map((s,i)=>(
            <div key={i} className="flex flex-col items-center gap-2 min-w-[120px]" aria-label={s.name} title={s.name}>
              <span className="hover:scale-125 transition-transform">
                {s.icon}
              </span>
